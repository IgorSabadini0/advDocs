...
import { clientModel } from '../models/clienteModel.js';
...
const clienteController = {
    listClientes: async (req, res) => {
        try {
            const clientes = await clientModel.list();
    ...
    createCliente: async (req, res) => {
        try {
            ...
            const registroExistente = await clientModel.verificarExistencia(numeroProc, numeroPasta); // Verifica se já existe um cliente com o mesmo número de processo ou número de pasta
            ...
            const resultado = await clientModel.create({
                acao,
                nome,
                numeroPasta,
                tipo,
            ...
            return res.status(200).json({ mensagem: "Cliente cadastrado com sucesso!", id: resultado.insertId });
        } catch (error) {
            onsole.error(`Erro ao editar registro: ${error}`);
            return res.status(500).json({ mensagem: "Erro interno no servidor" });
        }
    }
}
